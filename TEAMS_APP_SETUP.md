# Connect test-executor to Microsoft Teams

`azd deploy` already did the Azure side for you:

- Azure Bot: `test-executor-bot-e44a0835` (Microsoft Teams channel enabled)
- Bot ID (msaAppId): `9deace09-cec6-4b53-b072-7fa22df290d2`  <- you will paste this as the bot id
...
3. Left menu **App features** -> **Bot** -> **Select an existing bot** -> enter the
   Bot ID `9deace09-cec6-4b53-b072-7fa22df290d2`, tick the **Personal** scope, then **Save**.
...
```json
{
  ...
  "name": { "short": "test-executor", "full": "test-executor" },
  "description": { "short": "test-executor agent", "full": "test-executor agent on Microsoft Teams" },
  "icons": { "color": "color.png", "outline": "outline.png" },
  "accentColor": "#FFFFFF",
  "bots": [{ "botId": "9deace09-cec6-4b53-b072-7fa22df290d2", "scopes": ["personal"] }]
}
```
...
```sh
zip -j test-executor-teams-app.zip manifest.json color.png outline.png          # bash
```
```powershell
Compress-Archive manifest.json,color.png,outline.png test-executor-teams-app.zip # PowerShell
```
...
atk install --file-path test-executor-teams-app.zip --scope Personal
```
