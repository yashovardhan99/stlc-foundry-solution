# Connect agent-orchestrator-poc to Microsoft Teams

`azd deploy` already did the Azure side for you:

- Azure Bot: `agent-orchestrator-poc-bot-554edd02` (Microsoft Teams channel enabled)
- Bot ID (msaAppId): `7b9b55a5-2946-44b1-9ea2-58ede60f4d28`  <- you will paste this as the bot id
...
3. Left menu **App features** -> **Bot** -> **Select an existing bot** -> enter the
   Bot ID `7b9b55a5-2946-44b1-9ea2-58ede60f4d28`, tick the **Personal** scope, then **Save**.
...
```json
{
  ...
  "name": { "short": "agent-orchestrator-poc", "full": "agent-orchestrator-poc" },
  "description": { "short": "agent-orchestrator-poc agent", "full": "agent-orchestrator-poc agent on Microsoft Teams" },
  "icons": { "color": "color.png", "outline": "outline.png" },
  "accentColor": "#FFFFFF",
  "bots": [{ "botId": "7b9b55a5-2946-44b1-9ea2-58ede60f4d28", "scopes": ["personal"] }]
}
```
...
```sh
zip -j agent-orchestrator-poc-teams-app.zip manifest.json color.png outline.png          # bash
```
```powershell
Compress-Archive manifest.json,color.png,outline.png agent-orchestrator-poc-teams-app.zip # PowerShell
```
...
atk install --file-path agent-orchestrator-poc-teams-app.zip --scope Personal
```
